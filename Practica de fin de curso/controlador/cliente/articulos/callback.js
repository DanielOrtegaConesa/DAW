function callbackListar(datos) {
    ...
    for (let c in datos.resultados) {
        $("#contenedorArticulos").append(" " +
            ...
            "      <span>"+resultados[c]["precio"]+"&euro;</span> " +
            "    </div>" +
        ...
        /*
                $("#tb").append(
                    ...
                    "<td>" +
                    "   <a class='btn-floating btn-large waves-effect waves-light blue lighten-1' id='" + resultados[c]["codArticulo"] + "'><i class='material-icons'>add</i></a>" +
                    "<td/>" +
                    "</tr>"
                );*/
        muestrapag();
        ...
        $("#" + resultados[c]["codArticulo"]).click(function () {
            $.ajax({
                method: "GET",
                cache: false,
                url: "controlador/cliente/articulos/acciones.php",
